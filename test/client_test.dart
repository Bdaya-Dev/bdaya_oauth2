...
void main() {
  ...
  group('with expired credentials', () {
    ...
    test(
        ...
        'request', () async {
      var expiration = DateTime.now().subtract(const Duration(hours: 1));
      var credentials = oauth2.Credentials('access token', refreshToken: 'refresh token', tokenEndpoint: tokenEndpoint, expiration: expiration);
      var client = oauth2.Client(credentials, identifier: 'identifier', secret: 'secret', httpClient: httpClient);
    ...
    test('that onCredentialsRefreshed is called', () async {
      ...
      var credentials = oauth2.Credentials('access token', refreshToken: 'refresh token', tokenEndpoint: tokenEndpoint, expiration: expiration);
      var client =
          oauth2.Client(credentials, identifier: 'identifier', secret: 'secret', httpClient: httpClient, onCredentialsRefreshed: (credentials) {
        callbackCalled = true;
