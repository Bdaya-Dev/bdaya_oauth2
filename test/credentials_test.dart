...
void main() {
  ...
  test('can refresh with a refresh token and a token endpoint', () async {
    var credentials = oauth2.Credentials('access token', refreshToken: 'refresh token', tokenEndpoint: tokenEndpoint, scopes: ['scope1', 'scope2']);
    ...
    httpClient.expectRequest((request) {
      ...
      expect(request.bodyFields, equals({'grant_type': 'refresh_token', 'refresh_token': 'refresh token', 'scope': 'scope1 scope2'}));
      expect(request.headers, containsPair('Authorization', 'Basic aWQlQzMlQUJudCVDNCVBQmZpZXI6cyVDMyVBQmNyZXQ='));

      return Future.value(http.Response(
          jsonEncode({'access_token': 'new access token', 'token_type': 'bearer', 'refresh_token': 'new refresh token'}), 200,
  ...
  test('sets proper scope string when using custom delimiter', () async {
    var credentials =
        oauth2.Credentials('access token', refreshToken: 'refresh token', tokenEndpoint: tokenEndpoint, scopes: ['scope1', 'scope2'], delimiter: ',');
  ...
  test('can refresh without a client secret', () async {
    ...
    httpClient.expectRequest((request) {
      ...
      expect(request.bodyFields,
          equals({'grant_type': 'refresh_token', 'refresh_token': 'refresh token', 'scope': 'scope1 scope2', 'client_id': 'identifier'}));
  ...
  test('uses form-field authentication if basicAuth is false', () async {
    ...
    credentials = await credentials.refresh(identifier: 'idëntīfier', secret: 'sëcret', basicAuth: false, httpClient: httpClient);
    expect(credentials.accessToken, equals('new access token'));
    expect(credentials.refreshToken, equals('new refresh token'));
  });
  ...
  group('fromJson', () {
    ...
    test('should load the same credentials from toJson', () {
      ...
      var credentials = oauth2.Credentials('access token',
          refreshToken: 'refresh token', idToken: 'id token', tokenEndpoint: tokenEndpoint, scopes: ['scope1', 'scope2'], expiration: expiration);
